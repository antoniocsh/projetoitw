...
var vm = function () {
    ...
    self.displayName = 'Olympic Games Modalities';
    ...
    self.activate = function (id) {
        ...
        ajaxHelper(composedUri, 'GET').done(function (data) {
            ...
            for (var i = 0; i <= self.records().length; i++){
                self.updateheart((self.records()[i]).Id, 'modalities')
            }
        });
    };
    $().ready(function () {
        $("#tagsAthletes").autocomplete({
            ...
            select: function (event, ui) {
                window.location.href = "modalityDetails.html?id=" + ui.item.name;
            },
        ...
        $('#searchform').submit(function(event) {
            ...
            window.location.href = "modalityDetails.html?id=" + athleteID;
            } else {
            // if the ID is not valid, show an error message
            $('#error-message').html('<span class="text-danger"><i class="fa fa-warning" aria-hidden="true"></i> Invalid Modality ID</span>'); 
            }
          });
